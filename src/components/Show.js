...
const Show = () => {
  ...
  const getShowDetail = async () => {
    const response = await fetch(`${process.env.REACT_APP_BACKEND_URL}/api/show/${showId}`);
        ...
        <>
          ...
          <div className="flex flex-col">
            {seatCategory.arrangements?.map((row) => (
              <div className="flex">
                <button className="bg-transparent text-slate-500 py-2 px-4">
                  A
                </button>
                {row?.map((col) => (
                  <button
                    className={
                      (col.status !== "BOOKED"
                        ...
                        : "bg-slate-500 text-slate-200 disabled ") +
                      ` border border-slate-500 text-slate-500 py-2 px-4 m-2`
                    }
                  >
                    {col.seatNumber}
                  </button>
